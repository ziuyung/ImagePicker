...
- (NSString *)toolbarTitle
{
    ...
    {
        return [NSString stringWithFormat:NSLocalizedStringFromTable(@"picker.selection.multiple-items", @"GMImagePicker", @"%@ 项被选择" ), @(nImages+nVideos)];
    }
    else if (nImages>1)
    {
        return [NSString stringWithFormat:NSLocalizedStringFromTable(@"picker.selection.multiple-photos", @"GMImagePicker", @"%@ 张照片被选择"), @(nImages)];
    }
    else if (nImages==1)
    {
        return NSLocalizedStringFromTable(@"picker.selection.single-photo", @"GMImagePicker", @"已选择1张照片" );
    }
    else if (nVideos>1)
    {
        return [NSString stringWithFormat:NSLocalizedStringFromTable(@"picker.selection.multiple-videos", @"GMImagePicker", @"%@ 视频被选择"), @(nVideos)];
    }
    else if (nVideos==1)
    {
        return NSLocalizedStringFromTable(@"picker.selection.single-video", @"GMImagePicker", @"已选择1 项视频");
    }
    else
    {
        return nil;
